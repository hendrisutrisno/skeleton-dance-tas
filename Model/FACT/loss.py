...
class MatchCriterion():
    ...
    def set_label(self, label):
        ...
        if self._class_weight is not None:
            ...
            for i in self.bg_ids:
                sweight[self.transcript==i] = self.cfg.Loss.nullw
    ...
    def a2f_soft_iou(self, a2f_attn, onehot_seg_label):
        ...
        a2f_attn_np = a2f_attn.detach().cpu().numpy()
        onehot_seg_label_np = onehot_seg_label.detach().cpu().numpy()
    ...
    def _one_to_many_match(self, cost):
        transcript_np = self.transcript.detach().cpu().numpy()
